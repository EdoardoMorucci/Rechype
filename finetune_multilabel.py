...
from Code.Modules.Finetuning.A_train_multilabel import train
...
def finetune(
    ...
    train_dataset = MultiLabelDataset(
        dataset_path, "Train", augment=augment, sample_rate=sample_rate
    )
    valid_dataset = MultiLabelDataset(
        dataset_path, "Validation", sample_rate=sample_rate
    )
    ...
    class_weights = (
        train_dataset.labels["class"]
        .to_numpy()
        .reshape((len(train_dataset.labels), -1))
    )
    class_weights = 1 - class_weights.sum() / class_weights.sum().sum()
