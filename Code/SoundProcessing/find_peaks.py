...
def create_windows(
    ...
    left_indices = np.clip(left_indices, 0, len(waveform))
    right_indices = np.clip(right_indices, 0, len(waveform))
...
def find_peak_times(
    ...
    waveform_fourier = librosa.stft(waveform)

    # Take the absolute value
    magnitude_spectrum = np.abs(waveform_fourier)

    # Convert the amplitude spectrogram to dB-scaled spectrogram
    db = librosa.amplitude_to_db(magnitude_spectrum, ref=np.max)
